...
def back():
    global song_box, music_slider, music_label_text, loaded, previous_pitch
    ...
    song = f'{playlists_record.get_directory(song)}{song}.mp3'
...
def play():
    global paused, loaded, audio
    if not loaded:
        song = song_box.get(ACTIVE)
        song_index = song_box.index(ACTIVE)
        song = f'{playlists_record.get_directory(song)}{song}.mp3'
        loaded = True
        audio = PitchShifter(song, previous_pitch)
    #Play
    audio.play()
    paused = False
    update()
