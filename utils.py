...
class PhaseVocoder:
    ...
    def __init__(self, window_size, pitch_ratio):
        ...
        self.synthesis_hopsize = window_size//4
        self.analysis_hopsize = int(self.synthesis_hopsize//pitch_ratio)
        self.last_phase = np.zeros(window_size//2+1, dtype=np.float32) #frame n − 1
        self.accum_phase = np.zeros(window_size//2+1, dtype=np.float32)
        self.expected_phase = np.linspace(0, window_size//2, window_size//2+1)*2*np.pi*self.analysis_hopsize//window_size  # expected phase (2049,)
        self.last_magnitudes = np.zeros(window_size//2+1, dtype=np.float32) #frame n − 1
    def calc_phase(self, current_frame):
        """Saves previous phase values for calculation of next frame"""
        current_phase = np.angle(current_frame)
        current_magn = abs(current_frame)
